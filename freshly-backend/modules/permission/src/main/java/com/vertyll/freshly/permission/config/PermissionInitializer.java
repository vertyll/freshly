...
public class PermissionInitializer {
    ...
    public CommandLineRunner initializePermissions() {
        return _ -> {
            ...
            createMappingIfNotExists(UserRoleEnum.USER.getValue(), Permission.AUTH_CHANGE_PASSWORD);
            createMappingIfNotExists(UserRoleEnum.USER.getValue(), Permission.AUTH_CHANGE_EMAIL);
            createMappingIfNotExists(UserRoleEnum.USER.getValue(), Permission.USERS_READ);

            // ADMIN role permissions (all permissions)
            for (Permission permission : Permission.values()) {
                createMappingIfNotExists(UserRoleEnum.ADMIN.getValue(), permission);
            }
