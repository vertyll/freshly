...
class UserPermissionCacheTest {
    ...
    void shouldStripRolePrefixFromAuthorities() {
        ...
        when(rolePermissionRepository.findByKeycloakRoleIn(any()))
    ...
    void shouldReturnEmptySetWhenUserHasNoRoles() {
        ...
        when(rolePermissionRepository.findByKeycloakRoleIn(Set.of()))
                .thenReturn(List.of());
    ...
    void shouldReturnEmptySetWhenRolesHaveNoMappedPermissions() {
        ...
        when(authentication.getName()).thenReturn("testuser");
        Collection<GrantedAuthority> authorities = List.of(
                new SimpleGrantedAuthority("ROLE_norole")
        );
        when(authentication.getAuthorities()).thenAnswer(_ -> authorities);
        when(rolePermissionRepository.findByKeycloakRoleIn(Set.of("norole")))
                .thenReturn(List.of());
