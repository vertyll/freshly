 ...
 * This enum standardizes air quality level handling across the application.
 * MongoDB stores the enum name (e.g., "VERY_GOOD"), and the API returns the same.
 * The polishName is only used for parsing data from GIOŚ API.
 */
public enum AirQualityLevel {
    ...
    /**
     * -- GETTER --
     *  Returns severity level (0 = best, 5 = worst).
    ...
    /**
     * Returns the API value (enum name) to be sent to frontend.
     * Frontend should use this value for translations.
     */
    ...
    /**
     * Parses Polish name from GIOŚ API to enum value.
