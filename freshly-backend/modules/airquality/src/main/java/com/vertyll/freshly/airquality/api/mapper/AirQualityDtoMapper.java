...
public interface AirQualityDtoMapper {
    ...
    List<SensorMeasurementResponseDto> toSensorMeasurementResponseList(
    ...
    @Mapping(
            target = "overallIndexLevel",
            expression = "java(toApiValue(measurement.getOverallIndexLevel()))")
    @Mapping(
            target = "so2IndexLevel",
            expression = "java(toApiValue(measurement.getSo2IndexLevel()))")
    @Mapping(
            target = "no2IndexLevel",
            expression = "java(toApiValue(measurement.getNo2IndexLevel()))")
    @Mapping(
            target = "pm10IndexLevel",
            expression = "java(toApiValue(measurement.getPm10IndexLevel()))")
    @Mapping(
            target = "pm25IndexLevel",
            expression = "java(toApiValue(measurement.getPm25IndexLevel()))")
    AirQualityMeasurementResponseDto toAirQualityMeasurementResponse(
    ...
    List<StationDistanceResponseDto> toStationDistanceResponseList(
            List<StationDistance> stationDistances);
    ...
    @Mapping(
            target = "mostCommonQualityLevel",
            expression = "java(toApiValue(stats.getMostCommonQualityLevel()))")
    AirQualityStatisticsResponseDto toStatisticsResponse(AirQualityStatistics stats);

    default AirQualityStatisticsResponseDto.Pm10Statistics mapPm10Stats(
    ...
    default AirQualityStatisticsResponseDto.Pm25Statistics mapPm25Stats(
            AirQualityStatistics stats) {
        return new AirQualityStatisticsResponseDto.Pm25Statistics(
                stats.pm25Avg(), stats.pm25Min(), stats.pm25Max());
    }

    default AirQualityStatisticsResponseDto.OtherPollutants mapOtherPollutants(
            AirQualityStatistics stats) {
        return new AirQualityStatisticsResponseDto.OtherPollutants(
                stats.so2Avg(), stats.no2Avg(), stats.coAvg(), stats.o3Avg());
    }

    default AirQualityStatisticsResponseDto.QualityDistribution mapQualityDistribution(
            AirQualityStatistics stats) {
    ...
    // Ranking mappings
    @Mapping(
            target = "dominantQualityLevel",
            expression = "java(toApiValue(ranking.dominantQualityLevel()))")
